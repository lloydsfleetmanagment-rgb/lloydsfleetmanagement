...
AS $function$
DECLARE v_emp text; v_base text; v_role public.app_role; v_req text; v_i int := 0;
BEGIN
  ...
  v_req := lower(COALESCE(NEW.raw_user_meta_data->>'role',''));
  v_role := CASE
    WHEN v_req = 'admin' THEN 'admin'::public.app_role
    WHEN v_req = 'supervisor' THEN 'supervisor'::public.app_role
